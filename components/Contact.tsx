...
export default function Contact() {
  ...
  return (
    <section id="contact" style={{ maxWidth:"1320px",margin:"0 auto",padding: isMobile ? "80px 24px 60px" : "140px 48px 100px" }}>
      <div style={{ display:"grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: isMobile ? "48px" : "80px", alignItems:"end" }}>
        <div>
          <div className="num" style={{ marginBottom:"12px" }}>06</div>
          <h2 style={{ fontFamily:"var(--serif)",fontSize: isMobile ? "clamp(2.5rem,10vw,3.5rem)" : "clamp(2.5rem,5vw,5rem)",fontWeight:900,lineHeight:1.0,letterSpacing:"-0.04em",color:"var(--ink)",marginBottom:"28px" }}>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:"12px" }}>
          {[
            ...
            <a key={c.label} href={c.href} target={c.href.startsWith("http")?"_blank":undefined} rel="noopener noreferrer"
              style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"18px 20px",border:"1px solid var(--border2)",borderRadius:"3px",textDecoration:"none",transition:"all 0.2s",background:"transparent",gap:"16px" }}
            ...
            >
              <div>
                ...
                <div style={{ fontFamily:"var(--sans)",fontSize: isMobile ? "0.82rem" : "0.9rem",fontWeight:500,color:"var(--ink2)",wordBreak:"break-all" }}>{c.value}</div>
              </div>
              <span style={{ fontFamily:"var(--mono)",fontSize:"0.8rem",color:"var(--ink4)",flexShrink:0 }}>↗</span>
            </a>
          ))}

          <button onClick={copy} style={{
            cursor:"none",background:"none",border:"1px solid var(--border)",borderRadius:"3px",
            padding:"14px 20px",fontFamily:"var(--mono)",fontSize:"0.68rem",letterSpacing:"0.1em",
