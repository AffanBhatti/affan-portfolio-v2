      ...
      <div style={{ maxWidth:"1320px",margin:"0 auto",padding:"120px 48px" }}>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 2fr",gap:"80px",alignItems:"start" }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"48px 64px" }}>
            {groups.map(g=>(
              <div key={g.label}>
<div style={{ fontFamily:"var(--serif)",fontSize:"1rem",fontWeight:700,color:"var(--ink)",letterSpacing:"-0.01em",marginBottom:"16px",borderBottom:"1px solid var(--border2)",paddingBottom:"10px" }}>{g.label}</div>                <div style={{ display:"flex",flexDirection:"column",gap:"8px" }}>
                  {g.items.map(item=>(
                    <div key={item} style={{ display:"flex",alignItems:"center",gap:"10px" }}>
                      <span style={{ width:"4px",height:"4px",borderRadius:"50%",background:"var(--accent)",flexShrink:0 }}/>
                      <span style={{ fontFamily:"var(--sans)",fontSize:"0.88rem",color:"var(--ink2)" }}>{item}</span>
                    </div>
                  ))}
                </div>
              </div>
